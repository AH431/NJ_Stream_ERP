...
- [x] Agent A-2：Auth JWT 中間件 + 角色權限矩陣 #4
- [x] Agent A-3：POST /api/v1/sync/push 骨架 + 錯誤碼回傳 #5
- [x] Agent B-2：SyncProvider 骨架（Riverpod）+ 離線佇列 #6
...
- [x] Agent A：客戶 / 產品 REST API（含軟刪除 + 權限） #7
- [x] **Agent B（修正強化）**：客戶 / 產品列表 UI + 離線新增 + 軟刪除 #8
  - **強制規則**：UI 刪除一律使用 `deleted_at` 標記（不得 Hard Delete）
  - 同步時必須正確傳送 deleted_at 更新（LWW 判斷依據）
- [x] Agent B：LWW 衝突解決實作（updated_at 比較） #9

## Milestone: W5 CRM
- [x] Agent A：報價 API + 報價轉訂單（First-to-Sync wins） #10
- [x] Agent B：報價單 UI（稅額切換 + decimal） #11

## Milestone: W6–W8 SCM（最核心）
**Epic: DELTA_UPDATE 庫存邏輯**
- [x] Agent A：實作 DELTA_UPDATE 四種 type（reserve/cancel/out/in） #12
- [x] Agent A：INSUFFICIENT_STOCK 409 + Fail-to-Pull 機制 #13
- [x] **Agent A/B 共同（新增）**：併發衝突模擬測試（Race Condition） #14
...
**Epic: 訂單與出貨流程**
- [x] Agent B：確認訂單 → reserve UI 流程（不自動 reserve） #15
- [x] Agent B：出貨 UI（type: out） #16
- [x] **Agent Antigravity**：全域系統審計、合規性檢查與效能優化 #21
  - 修復 `GET /pull` 明細缺失漏洞
