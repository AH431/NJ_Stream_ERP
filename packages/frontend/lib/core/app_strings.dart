...
class AppStrings extends ChangeNotifier {
  ...
  String get snackStockInQueued =>
      _s('入庫已排入待同步佇列，同步後庫存將更新',
  ...
  String get errEmptyCredentials  => _s('請輸入帳號與密碼', 'Please enter username and password.');
  String get errLoginFailed       => _s('登入失敗，請檢查帳號密碼。', 'Login failed. Please check your credentials.');
  ...
  String dashOnHandReserved(int onHand, int reserved) =>
      _isEnglish ? 'On Hand: $onHand  Reserved: $reserved' : '庫存 $onHand　預留 $reserved';
  String dashSafetyShortage(int min, int shortage) =>
      _isEnglish ? 'Safety: $min  Shortage: $shortage' : '安全庫存 $min　缺 $shortage';

  // ── Customer List ─────────────────────────────────────────────────────────
  String get custEmptyTitle   => _s('尚無客戶資料',                'No customers yet.');
  String get custEmptyAdd     => _s('點擊右下角 ＋ 新增第一位客戶', 'Tap ＋ to add your first customer.');
  ...
  String custDelBody(String name) =>
      _isEnglish ? 'Delete "$name"? This cannot be undone.'
                 : '確定要刪除「$name」？此操作無法復原。';
  ...
  String prodDelBody(String name) =>
      _isEnglish ? 'Delete "$name"? This cannot be undone.'
                 : '確定要刪除「$name」？此操作無法復原。';
  ...
  String quotLabelTax(bool withTax) =>
      _isEnglish ? (withTax ? 'Tax (5%): ' : 'Tax: ') : (withTax ? '稅額（5%）：' : '稅額：');
  ...
  String invMinStock(int level) =>
      _isEnglish ? 'Min: $level' : '最低庫存閾值：$level';
